...
use super::location::{Location, Type as LocationType};
...
impl Unitfile {
    ...
    pub async fn search(&self, loc: Location) -> SearchResult {
        dbg!(&loc);
        match loc.get_type() {
            LocationType::Script => {
                Ok(self.units.get(&loc.get_path_string()).map(|s| s.clone()))
            },
            _ => Err(loc.err("Path component following a unitfile must be a script name"))
        }
    }
}
